...
if (empty($_SESSION['article_edit_csrf_token'])) {
    $_SESSION['article_edit_csrf_token'] = bin2hex(random_bytes(32));
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $csrf_token = $_POST['csrf_token'] ?? '';
    
    if (!hash_equals($_SESSION['article_edit_csrf_token'], $csrf_token)) {
        ...
        if (empty($title) || empty($description)) {
            ...
            $stmtSlug = $pdo->prepare("SELECT COUNT(*) FROM `articles` WHERE `slug` = :slug AND `id` != :id");
            ...
            // Handle Image Upload
            $imagePath = $article['image']; // Default to old image
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($fileExtension, $allowedExtensions)) {
                    ...
                    if (move_uploaded_file($fileTmpPath, $dest_path)) {
                        // Delete old uploaded image file if it exists and is not a default asset
                        if ($article['image'] && strpos($article['image'], 'uploads/') !== false) {
                            $oldFilePath = __DIR__ . '/../' . $article['image'];
            ...
            if (empty($error_msg)) {
                try {
                    ...
                    error_log('[Article Edit Error] ' . $e->getMessage());
                    $error_msg = APP_DEBUG ? 'Database Error: ' . $e->getMessage() : 'An error occurred while updating the article.';
                }
            }
        }
    }
...
<div class="card card-primary card-outline">
    <div class="card-header">
        <h3 class="card-title font-weight-bold" style="color: var(--sidebar-color);"><i class="fas fa-edit mr-1"></i> Edit Article</h3>
    </div>
    
    <form action="article-edit.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data" class="form-horizontal">
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['article_edit_csrf_token']) ?>">
        ...
        <div class="card-body">
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-4">
                    <select class="form-control" name="type" id="type" onchange="updateSchema()">
                        <option value="news" <?= $article['type'] === 'news' ? 'selected' : '' ?>>News & Stories</option>
                        <option value="blog" <?= $article['type'] === 'blog' ? 'selected' : '' ?>>Blog Post</option>
                    </select>
                </div>
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="title" id="title" placeholder="e.g. Bangalore Kitchen Launch Event" value="<?= htmlspecialchars($article['title']) ?>" required>
                </div>
            </div>
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="slug" id="slug" placeholder="e.g. bangalore-kitchen-launch-event" value="<?= htmlspecialchars($article['slug']) ?>" required>
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-10">
                    ...
                    <small class="form-text text-muted">Upload a new article banner image to replace the existing one (Formats: JPEG, PNG, WEBP. Max 2MB).</small>
                    
                    <div class="img-preview-container" id="previewContainer">
                        <img src="<?= !empty($article['image']) ? '../' . htmlspecialchars($article['image']) : '' ?>" alt="preview" class="img-preview" id="imagePreview">
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-10">
                    <textarea name="description" id="description" rows="10" placeholder="Write your article content here..." required><?= htmlspecialchars($article['description']) ?></textarea>
                </div>
            </div>
            ...
            <div class="seo-section">
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control form-control-sm" name="meta_title" placeholder="Search engine title tag. If left blank, will fallback to article title." value="<?= htmlspecialchars($article['meta_title'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-sm-10">
                        <textarea class="form-control form-control-sm" name="meta_description" rows="2" placeholder="Brief summary of the article for Google search snippet."><?= htmlspecialchars($article['meta_description'] ?? '') ?></textarea>
                    </div>
                </div>
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control form-control-sm" name="meta_keywords" placeholder="Comma-separated keywords (e.g. food charity, volunteer stories)" value="<?= htmlspecialchars($article['meta_keywords'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="form-group row mb-0">
                    ...
                    <div class="col-sm-10">
                        <textarea class="form-control form-control-sm text-monospace" name="schema_json" id="schema_json" rows="4" style="font-size: 12px;" placeholder="Schema JSON text..."><?= htmlspecialchars($article['schema_json'] ?? '') ?></textarea>
        ...
        <div class="card-footer text-right">
            <button type="submit" class="btn btn-success" style="background-color: var(--primary-green) !important; border-color: var(--primary-green) !important; padding: 8px 30px;"><i class="fas fa-save mr-1"></i> Update & Publish</button>
        </div>
    </form>
</div>
...
<script>
var schemaTextareaDirty = <?= !empty($article['schema_json']) ? 'true' : 'false' ?>;
...
function previewImage(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        // If file input cleared, fallback to existing database image if any
        <?php if (!empty($article['image'])): ?>
            preview.src = '../<?= htmlspecialchars($article['image']) ?>';
            container.style.display = 'block';
        <?php else: ?>
            preview.src = '';
...
function updateSchema() {
    ...
    <?php if (!empty($article['image'])): ?>
        imageURL = 'https://globalrisefoundation.org/<?= htmlspecialchars($article['image']) ?>';
    <?php endif; ?>
